  ...
  Widget build(BuildContext context, WidgetRef ref) {
    return ValueListenableBuilder<Map<String, dynamic>>(
      ...
      builder: (context, filters, child) {
        final minPrice = filters['min_price'] ?? 0;
        final maxPrice = filters['max_price'] ?? 10000;
        final currentMinPrice = useState(minPrice);
        final currentMaxPrice = useState(maxPrice);
        
        // Обновляем состояния при изменении фильтров
        useEffect(() {
          currentMinPrice.value = filters['min_price'] ?? 0;
          currentMaxPrice.value = filters['max_price'] ?? 1000;
          return null;
        ...
        return Column(
          ...
          children: [
            ...
            RangeSlider(
              min: 0,
              max: 10000,
              divisions: 10, // Шаг 100 руб, так как (10000-0)/100 = 100
              values: RangeValues(
