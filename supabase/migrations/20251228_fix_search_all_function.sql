-- Fix the search_all function to handle grape varieties through the wine_grape_varieties and grape_varieties tables
...
BEGIN
    ...
    WITH wine_data AS (
        ...
        GROUP BY w.id, w.winery_id, w.name, w.description, w.image_url, w.color, w.type, w.sugar, w.vintage, w.alcohol_level, w.rating, w.average_rating, w.reviews_count, w.serving_temperature, w.sweetness, w.acidity, w.tannins, w.saturation, w.created_at, w.updated_at, w.is_deleted, o.price, wn.id, wn.name, wn.description, wn.logo_url, wn.banner_url, wn.winemaker, wn.website, wn.location_text, wn.country_code, c.name, wn.region
    ),
    aggregated_wine_data AS (
        SELECT
            ...
            winery_region
        FROM wine_data
        GROUP BY id, winery_id, name, description, grape_varieties, image_url, color, type, sugar, vintage, alcohol_level, rating, average_rating, reviews_count, serving_temperature, sweetness, acidity, tannins, saturation, created_at, updated_at, is_deleted, winery_id_field, winery_name, winery_description, winery_logo_url, winery_banner_url, winery_winemaker, winery_website, winery_location_text, winery_country_code, country_name, winery_region
    )
    SELECT jsonb_agg(
