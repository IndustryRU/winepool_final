-- Обновляем функцию get_wines_with_prices для поддержки show_unavailable и добавления min_price/max_price
...
BEGIN
    ...
    query_text := query_text || '
        ...
        aggregated_wine_data AS (
            SELECT
                ...
                country_name
            FROM wine_data
            GROUP BY id, winery_id, name, description, grape_variety, image_url, color, type, sugar, vintage, region, alcohol_level, average_rating, created_at, updated_at, is_deleted, winery_id_field, winery_name, winery_country_code, country_code, country_name
        )
    ...
    IF filters ? 'region' THEN
        IF jsonb_typeof(filters->'region') = 'array' THEN
            query_text := query_text || ' AND awd.region = ANY(ARRAY(SELECT jsonb_array_elements_text($1->''region'')))';
        ELSE
            query_text := query_text || ' AND awd.region = ($1->>''region'')';
        END IF;
    END IF;
    ...
    query_text := query_text || '
        ...
        GROUP BY awd.id, awd.winery_id, awd.name, awd.description, awd.grape_variety, awd.image_url, awd.color, awd.type, awd.sugar, awd.vintage, awd.region, awd.alcohol_level, awd.average_rating, awd.created_at, awd.updated_at, awd.is_deleted, awd.min_price, awd.max_price, awd.winery_id_field, awd.winery_name, awd.winery_country_code, awd.country_code, awd.country_name';
