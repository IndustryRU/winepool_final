DROP FUNCTION IF EXISTS add_to_user_storage(uuid, integer, numeric, timestamp with time zone, integer, integer);
...
BEGIN
  ...
  SET
    quantity = user_storage.quantity + EXCLUDED.quantity,
    purchase_price = COALESCE(EXCLUDED.purchase_price, user_storage.purchase_price),
    purchase_date = COALESCE(EXCLUDED.purchase_date, user_storage.purchase_date),
    ideal_drink_from = COALESCE(EXCLUDED.ideal_drink_from, user_storage.ideal_drink_from),
    ideal_drink_to = COALESCE(EXCLUDED.ideal_drink_to, user_storage.ideal_drink_to),
    updated_at = NOW();
END;
$$ LANGUAGE plpgsql;
