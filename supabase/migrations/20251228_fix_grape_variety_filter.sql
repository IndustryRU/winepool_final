-- Исправляем функцию get_wines_with_prices, заменяя фильтрацию по w.grape_variety на фильтрацию через таблицу wine_grape_varieties
...
BEGIN
    ...
    -- ИСПРАВЛЕНИЕ: Заменяем фильтрацию по w.grape_variety на фильтрацию через wine_grape_varieties
    IF filters ? 'grape' THEN
        IF jsonb_typeof(filters->'grape') = 'array' THEN
            query_text := query_text || ' AND gv.name = ANY(ARRAY(SELECT jsonb_array_elements_text($1->''grape'')))';
        ELSE
            query_text := query_text || ' AND gv.name = ($1->>''grape'')';
        END IF;
    END IF;
    ...
    query_text := query_text || '
        GROUP BY w.id, w.winery_id, w.name, w.description, w.image_url, w.color, w.type, w.sugar, w.vintage, w.alcohol_level, w.average_rating, w.created_at, w.updated_at, w.is_deleted, o.price, wn.id, wn.name, wn.country_code, wn.region_id, c.code, c.name, r.name
        ),
        aggregated_wine_data AS (
            SELECT
                ...
                region_name
            FROM wine_data
            GROUP BY id, winery_id, name, description, grape_varieties, image_url, color, type, sugar, vintage, alcohol_level, average_rating, created_at, updated_at, is_deleted, winery_id_field, winery_name, winery_country_code, winery_region_id, country_code, country_name, region_name
        )
    ...
    -- ИСПРАВЛЕНИЕ: Заменяем фильтрацию по awd.grape_variety на фильтрацию по awd.grape_varieties
    IF filters ? 'grape' THEN
        IF jsonb_typeof(filters->'grape') = 'array' THEN
            query_text := query_text || ' AND ($1->''grape'')::jsonb ? ANY(awd.grape_varieties)';
        ELSE
            query_text := query_text || ' AND awd.grape_varieties ? ($1->>''grape'')';
        END IF;
    END IF;
    ...
    query_text := query_text || '
        ...
        GROUP BY awd.id, awd.winery_id, awd.name, awd.description, awd.grape_varieties, awd.image_url, awd.color, awd.type, awd.sugar, awd.vintage, awd.alcohol_level, awd.average_rating, awd.created_at, awd.updated_at, awd.is_deleted, awd.min_price, awd.max_price, awd.winery_id_field, awd.winery_name, awd.winery_country_code, awd.winery_region_id, awd.country_code, awd.country_name, awd.region_name';
